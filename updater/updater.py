...
from wknd_tools.UI import updater_ui
from wknd_tools.utils import reconnect_shaders

imp.reload(updater_ui)
...
def update_all_outdated():
    ...
    if not outdated:
        ...
        cmds.confirmDialog(title='Updated', message="No hay referencias desactualizadas! ^^")
        return

    progress = updater_ui.create_progress_window(len(outdated))

    print(f"\nActualizando {len(outdated)} referencia(s)...\n")

    updates = []
    try:
        for i, ref_data in enumerate(outdated, 1):

            # ACTUALIZAR PROGRESO
            ref_name = ref_data["entity_info"]["code"]
            progress.update_progress(i, f"Actualizando: {ref_name}")
    ...
    updater_ui.show_updates(updates)
